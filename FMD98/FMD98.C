/* FM MUSIC SAMPLE for PC-98 */
/* WatcomC */
/* 参考 PC-9801マシン語サウンドプログラミング、PC-9801プログラマーズBible */
...
unsigned char TONE[MAX_PARTSUU] = {1,1,1,1,1,1,1,1,1}; // 音長カウンタ
...
unsigned char FADEVOL = 0; // フェードアウトレベル
...
/* 音量設定 */
void set_fm_vol_op(char no, char reg, char ch, char vol)
{
	unsigned char m_vol;
	char j, bank = 0;
	if(ch >= 3){
	...
	m_vol = tone_table[no][FM_REG_TL1 + reg];
	m_vol += ((15 - vol) * 8);
	if(m_vol > 127)
		m_vol = 127;
	set_fm(bank, 0x40 + reg * 4 + j, m_vol);
}

void set_fm_vol(char no, char ch, char vol)
{
	switch(tone_table[no][FM_REG_FBCONNECT] & 0x07){
		case 7:
			set_fm_vol_op(no, 0, ch, vol);	/* Op 1 */

		case 5:
		case 6:
			set_fm_vol_op(no, 1, ch, vol);	/* Op 3 */

		case 4:
			set_fm_vol_op(no, 2, ch, vol);	/* Op 2 */
	
		case 0:
		case 1:
		case 2:
		case 3:
			set_fm_vol_op(no, 3, ch, vol);	/* Op 4 */
			break;
	}
}
...
void putvol(char i)
{
	...
	if(!STOPFRG[i])
		vol = 0;
	if(MAXVOL[i] >= 16)
		if(i >= (PARTSUU - 3)){	/* ENV,PSG */
			vol = 16;
			set_fm(0, 13, MAXVOL[i] - 16);
		}else
			vol = 15;
	else
		vol = MAXVOL[i];
	...
	if(i < PARTSUU - 3){
		set_fm_vol(TONE[i], i, vol);	/* FM */
	}else{
		set_fm(0, 8 + i - 3, vol);		/* PSG */
	}
}
...
void init_int(void)
{
	...
	outp(IMR_S, inp(IMR_S) & 0xef);	/* INT5 */
...
int	main(int argc,char **argv)
{
	...
	if(inp(0x18a) != 1){
		...
		if(inp(0x18a) == 0x01){
			printf("0x188 = OPNA ");
//			mem[WORK_PART] = 9;
			printf("0xa460 = %.2x ",inp(0xa460));
			outp(0xa460, (inp(0xa460) & 0xfc) | 1);
			set_fm(0, 0x29, 0x82);
			PARTSUU = 9;
		}else{
			printf("0x188 = OPN ");
